...
def test_dykstra_on_rounded_boxes():
    ...
    max_iter = 2
    converged_error = 1e-3
    projection, path, error_tuple, errs_to_plot = dykstra_projection(
        z, np.vstack([N_box1, N_box2]), np.hstack([c_box1, c_box2]), max_iter,
        track_error=True, plot_errors=True)
    ...
    plot_half_spaces(Nc_pairs, max_iter, ax=ax1)

    # Plot the path and points on the first subplot
    plot_path(path, ax1, errs_to_plot, True)
    ...
    ax1.legend()
    ax1.set_title(f"Modified Dykstra's method executed for {max_iter} iterations")
    ax1.set_xlabel("x")
    ax1.set_ylabel("y")
    ax1.grid(True)
