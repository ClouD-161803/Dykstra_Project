...
In this section, we propose a modification to Dykstra's algorithm to resolve the stalling problem in the general case. This represents the main contribution of this paper.
...
\subsection{Stall-Averse Dykstra} \label{subsec:algo}

% \begin{algorithm}[h!]
% \caption{Dykstra's Algorithm with Stall Fast-Forwarding}
% \label{alg:fastforward}
% \begin{algorithmic}[1]
% \REQUIRE Initial point $x^\circ$, half-spaces $\{f_i, c_i\}_{i=0}^{n-1}$, max iterations $N_{\max}$, stall tolerance $\epsilon_{\text{stall}}$
% \STATE \textbf{Initialise:} $x \leftarrow x^\circ$, $e_j \leftarrow \vec{0}$ for $j=0 \dots n-1$
% \STATE $X_{\text{hist}} \leftarrow [x]$
% \STATE $m \leftarrow 0$
% \WHILE{$m < N_{\max}$}
%     \STATE $i \leftarrow m \pmod n$
%     \STATE $x_{\text{target}} \leftarrow x + e_i$
%     \STATE $x_{\text{new}} \leftarrow \mathcal{P}_{\mathcal{H}_i}(x_{\text{target}})$ \COMMENT{Project using~\eqref{eq:dykstra:proj:poly}}
%     \STATE $e_i \leftarrow x_{\text{target}} - x_{\text{new}}$ \COMMENT{Update auxiliary vector}
%     \STATE $x \leftarrow x_{\text{new}}$
%     \STATE $X_{\text{hist}}.append(x)$
%     \STATE $m \leftarrow m + 1$
%     \IF{$m \ge n$ \AND $\twonorm{x - X_{\text{hist}}[m-n]} < \epsilon_{\text{stall}}$}
%         \STATE \COMMENT{Stalling detected}
%         \STATE $N_{stall} \leftarrow \text{ComputeStallLength}(m, \{e_j\}, \{f_j\}, \{c_j\}, n)$ \COMMENT{Using Thm.~\ref{thm:nstall}}
%         \STATE $N_{\text{cycles}} \leftarrow \lceil N_{stall} / n \rceil$
%         \FOR{$j \in \{0, \dots, n-1\}$}
%             \STATE $k_j \leftarrow \trans{f_j} e_j$ \COMMENT{Get scalar from $e_j = k_j f_j$}
%             \STATE $\Delta k_j \leftarrow \trans{f_j} x - c_j$ \COMMENT{$x$ is the constant stalling point}
%             \STATE $k_j \leftarrow k_j + N_{\text{cycles}} \times \Delta k_j$
%             \STATE $e_j \leftarrow k_j f_j$ \COMMENT{Update auxiliary vector}
%         \ENDFOR
%         \STATE $X_{\text{hist}}.extend([x] \times N_{stall})$ \COMMENT{Fill history}
...
This is illustrated for the line-box example in Figure~\ref{fig:stalling2}, which replicates the stalling situation from~\cite{DYKSTRASTALLING} and the fast-forwarding is applied in Figure~\ref{fig:fastforwarding}. The first column illustrates the trajectories of the iterates and their distance to the Eucledian projection computed using an interior point method. The second column shows the half-space activity, where half-space 0 corresponds to the left side of the box, half-space 1 to the top side of the box, and half-space 2 to the line (down-facing normal). In Figure~\ref{fig:stalling2}, the algorithm stalls until iteration 16. From the third plot, it can be seen that the algorithm stalls until half-space 0 becomes inactive. 
...
\begin{figure}
    ...
    \caption{Stalling situation. Standard Dykstra's algorithm applied to a line-box example. (Top): The projection path (blue) starts at the original point (blue dot) and projects onto each of the half-spaces. The iterates visibly stall near the left side of the box before converging to the optimal solution (red star). (Middle): The squared error (red) plateaus during the stalling period (yellow), which lasts from iteration 1 until iteration 16, before converging at iteration 30. (Bottom): Half-space activity. The stalling period ends precisely when half-space 0 (black circles), corresponding to the left side of the box, becomes inactive. Half-space 1 (black squares) and half-space 2 (black triangles) correspond to the top of the box and the line, respectively, and remain active.}
    \label{fig:stalling2}
\end{figure}
