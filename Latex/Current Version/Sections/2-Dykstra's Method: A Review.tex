...
In this paper, we restrict ourselves to the specific case of $X = \Rset^p$ and the norm $\twonorm{x}\eqdef (x_1^2+\dots+x_p^2)^{1/2}$. The Euclidean projection of $x^\circ\in\Rset^p$ onto a closed convex set $\mathcal{H}\subset \Rset^p$ is defined as the point $x^\star\coloneqq \mathcal{P}_{\mathcal{H}}(x)$ that minimises the Euclidean distance between $x^\circ$ and any point in $\mathcal{H}$. Here, it is assumed that $\mathcal{H}$ is represented as the intersection of $n$ closed convex sets $\mathcal{H}_i\subset \Rset^p$,
\begin{align}\label{eq:intersection}
\mathcal{H}=\bigcap_{i=0}^{n-1}\mathcal{H}_i,
\end{align}
so that $\mathcal{P}_{\mathcal{H}}(x)$ can be written as
...
The method of alternating projections (MAP) is an iterative method to find an $x^\star\in\Rset^p$ satisfying $x\in\bigcap_{i=0}^{n-1}\mathcal{H}_i$ by cyclically projecting onto the convex sets $\mathcal{H}_i$~\cite{NEUMANN,BREGMAN1965}. Starting from an arbitrary point $x_m=x^\circ$, MAP generates a sequence $\{x_m\}$ by computing
\begin{align}\label{eq:map}
    x_{m+1} = (P_{\mathcal{H}_{n-1}} \circ \dots \circ P_{\mathcal{H}_0}) (x_m).
\end{align}
When each $\mathcal{H}_i$ is an affine subspace, it can be shown that MAP also solves~\eqref{eq:projection}. For general closed convex sets, however, the sequence generated by~\eqref{eq:map} is only guaranteed to converge to a point within the intersection $\mathcal{H}$~\cite{BREGMAN1965}. This limit point is generally not the unique solution to~\eqref{eq:projection}.
...
Dykstra's method extends~\eqref{eq:map} to generate a sequence of iterates that converges to the solution of~\eqref{eq:projection}. Given $n$ convex sets $\mathcal{H}_0,\dots,\mathcal{H}_{n-1}$, Dykstra's alternating projection algorithm finds the orthogonal projection $x^\star$ of $x^\circ$ onto $\mathcal{H}\eqdef{\bigcap}_{i=0}^{n-1} \mathcal{H}_i$ by generating a series of iterates \{$x_{m}$\} using the scheme:
\begin{subequations}\label{eq:dykstra}
\begin{align}
x_{m+1}&=\mathcal{P}_{[m]}\left(x_{m}+e_{m-n}\right),\label{eq:dykstra:proj}\\
...
&e_{-n} = e_{-(n-1)} = ... = e_{-1} = 0.
\end{align}
The Boyle-Dykstra theorem~\cite{DYKSTRA} implies that $\lim_{m\rightarrow\infty}\anynorm{x_m-\mathcal{P}_\mathcal{H}(x^\circ)}=0$~\cite{DYKSTRA,DYKSTRAPERKINS}. For a finite number of iterations, there is no guarantee that $x_m\in\mathcal{H}$ nor that $x_m\neq x^\circ$. Note that~\eqref{eq:map} can be obtained from~\eqref{eq:dykstra} by setting $e_m=0\,\,\forall m$ in~\eqref{eq:dykstra:error}. 

\subsection{Connection to Fenchel Dual and ADMM} \label{subsec:admm_connection}

It was established that Dykstra's algorithm is equivalent to a block coordinate descent method applied to the Fenchel dual of the BAP~\cite{HAN1988,GAFFKE1989}. This duality arises from reformulating~\eqref{eq:bap} using the sum of the indicator functions $\delta_{\mathcal{H}_i}(x)$ for each constraint set:
\begin{equation}
